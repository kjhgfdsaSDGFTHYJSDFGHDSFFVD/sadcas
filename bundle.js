! function(e) {
    ...
    function Ln(e, t, n) {
        void 0 != e && (O.XP = e), void 0 != t && (O.maxXP = t), void 0 != n && (O.age = n), n == a.maxAge ? (Le.innerHTML = "MAX AGE", He.style.width = "100%") : (Le.innerHTML = "AGE " + O.age, He.style.width = O.XP / O.maxXP * 100 + "%")
    }
                ...
                function() {
                    ...
                    for (we.globalAlpha = 1, we.fillStyle = "rgba(0, 0, 70, 0.35)", we.fillRect(0, 0, oe, se), we.strokeStyle = nt, s = 0; s < W.length + q.length; ++s)
                        if ((D = W[s] || q[s - W.length]).visible && (10 != D.skinIndex || D == O || D.team && D.team == O.team || 10 == D.skinIndex)) {
                            ...
                            D.health > 0 && (a.healthBarWidth, we.fillStyle = nt, we.roundRect(D.x - f - a.healthBarWidth - a.healthBarPad, D.y - d + D.scale + a.nameY, 2 * a.healthBarWidth + 2 * a.healthBarPad, 17, 8), we.fill(), we.fillStyle = D == O || D.team && D.team == O.team ? "#8ecc51" : "#cc5151", we.roundRect(D.x - f - a.healthBarWidth, D.y - d + D.scale + a.nameY + a.healthBarPad, 2 * a.healthBarWidth * (D.health / D.maxHealth), 17 - 2 * a.healthBarPad, 7), we.fill(), we.textAlign="center", we.fillText(`\nHP:${D.health}/${D.maxHealth}`,D.x - f - a.healthBarWidth - a.healthBarPad+(2 * a.healthBarWidth + 2 * a.healthBarPad/2),D.y - d + (D.scale*1.3) + a.nameY))
                        }
